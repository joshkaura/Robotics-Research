...
**laser_robot_calibration.py** - Program for calibrating a laser distance sensor with a (FANUC) robot Tool Centre Point: finds the translation from an (x,y) sensor coordinate to an (x,y,z,w,p,r) robot coordinate.
...
**scan_analysis.py** - geometric feature analysis of laser distance profiles of various welding joints (currently V Grooves and Narrow Gaps). Uses data cleaning and (RANSAC) regression methods for accurately finding edges and corners within noisy signal data with significant numbers of grouped outliers (usually a product of laser lines on shiny, reflective surfaces).

![Pending](Results/pending.png)
